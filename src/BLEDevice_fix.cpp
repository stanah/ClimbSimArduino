...
std::vector<std::string> BLEDevice::clientAddress;
 ...
 */
/* STATIC */ void BLEDevice::gattServerEventHandler(esp_gatts_cb_event_t event,
                                                    esp_gatt_if_t gatts_if,
                                                    esp_ble_gatts_cb_param_t* param) {
  log_d("gattServerEventHandler [esp_gatt_if: %d] ... %s", gatts_if,
        BLEUtils::gattServerEventTypeToString(event).c_str());

  BLEUtils::dumpGattServerEvent(event, gatts_if, param);
  switch (event) {
    case ESP_GATTS_CONNECT_EVT: {
      if (BLEDevice::isClientDevice(param->connect.remote_bda)) return;
    ...
    case ESP_GATTS_DISCONNECT_EVT: {
      if (BLEDevice::isClientDevice(param->disconnect.remote_bda)) return;
      break;
 ...
 * @param [in] param
 */
/* STATIC */ void BLEDevice::gattClientEventHandler(esp_gattc_cb_event_t event,
                                                    esp_gatt_if_t gattc_if,
                                                    esp_ble_gattc_cb_param_t* param) {
  log_d("gattClientEventHandler [esp_gatt_if: %d] ... %s", gattc_if,
  ...
  for (auto& myPair : BLEDevice::getPeerDevices(true)) {
    ...
    if (((BLEClient*)conn_status.peer_device)->getGattcIf() == gattc_if ||
        ((BLEClient*)conn_status.peer_device)->getGattcIf() == ESP_GATT_IF_NONE ||
 ...
 */
/* STATIC */ void BLEDevice::gapEventHandler(esp_gap_ble_cb_event_t event,
  ...
  switch (event) {
    ...
    case ESP_GAP_BLE_NC_REQ_EVT: /*  NUMERIC CONFIRMATION  */
      ...
      if (BLEDevice::m_securityCallbacks != nullptr) {
        esp_ble_confirm_reply(
            ...
            BLEDevice::m_securityCallbacks->onConfirmPIN(param->ble_security.key_notif.passkey));
      }
#endif  // CONFIG_BLE_SMP_ENABLE
      break;
    case ESP_GAP_BLE_PASSKEY_REQ_EVT: /* passkey request event */
      ...
      if (BLEDevice::m_securityCallbacks != nullptr) {
        esp_ble_passkey_reply(param->ble_security.ble_req.bd_addr, true,
                              BLEDevice::m_securityCallbacks->onPassKeyRequest());
      }
#endif  // CONFIG_BLE_SMP_ENABLE
      break;
      /*
    ...
    case ESP_GAP_BLE_SEC_REQ_EVT:
      ...
      request. If not accept the security request, should sent the security response with
      negative(false) accept value*/
      log_i("ESP_GAP_BLE_SEC_REQ_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE  // Check that BLE SMP (security) is configured in make menuconfig
      if (BLEDevice::m_securityCallbacks != nullptr) {
        esp_ble_gap_security_rsp(param->ble_security.ble_req.bd_addr,
                                 BLEDevice::m_securityCallbacks->onSecurityRequest());
      } else {
       ...
       *
       */
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  // the app will receive this evt when the IO  has Output
                                         // capability and the peer device IO has Input
                                         // capability.
...
#ifdef CONFIG_BLE_SMP_ENABLE  // Check that BLE SMP (security) is configured in make menuconfig
      log_i("key type = %s",
            BLESecurity::esp_key_type_to_str(param->ble_security.ble_key.key_type));
#endif  // CONFIG_BLE_SMP_ENABLE
      break;
 ...
 */
/* STATIC */ std::string BLEDevice::getValue(BLEAddress bdAddress, BLEUUID serviceUUID,
                                             BLEUUID characteristicUUID) {
  log_v(">> getValue: bdAddress: %s, serviceUUID: %s, characteristicUUID: %s",
        bdAddress.toString().c_str(), serviceUUID.toString().c_str(),
...
/* STATIC */ void BLEDevice::init(std::string deviceName) {
  ...
  vTaskDelay(200 / portTICK_PERIOD_MS);  // Delay for 200 msecs as a workaround to an apparent
                                         // Arduino environment issue.
}  // init
 ...
 * @param [in] characteristicUUID
 */
/* STATIC */ void BLEDevice::setValue(BLEAddress bdAddress, BLEUUID serviceUUID,
                                      BLEUUID characteristicUUID, std::string value) {
  log_v(">> setValue: bdAddress: %s, serviceUUID: %s, characteristicUUID: %s",
        bdAddress.toString().c_str(), serviceUUID.toString().c_str(),
        characteristicUUID.toString().c_str());
  BLEClient* pClient = createClient();
  pClient->connect(bdAddress);
...
/*
 * @brief Set callbacks that will be used to handle encryption negotiation events and
...
/* STATIC */ void BLEDevice::deinit(bool release_memory) {
  ...
  if (release_memory) {
    esp_bt_controller_mem_release(
        ESP_BT_MODE_BTDM);  // <-- require tests because we released classic BT memory and this
                            // can cause crash (most likely not, esp-idf takes care of it)
  } else {
    initialized = false;
  }
#endif
...
void BLEDevice::setClientDevice(BLEAddress addr) { clientAddress.push_back(addr.toString()); }
void BLEDevice::unsetClientDevice(BLEAddress addr) {
  // TODO
  return;
}
bool BLEDevice::isClientDevice(BLEAddress addr) {
  for (int i = 0; i < clientAddress.size(); i++) {
    if (addr.toString() == clientAddress.at(i)) {
      return true;
    }
  }
  return false;
